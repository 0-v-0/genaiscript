...
export default function (ctx: ChatGenerationContext) {
    ...
    defAgent(
        ...
        async (ctx) => {
            ctx.$`Your are a helpful LLM agent that is an expert at Technical documentation. You can provide the best analysis to any query about the documentation.
...
FOLLOW THE SYNTAX PRECISELY. THIS IS IMPORTANT.
DIFF ./file.ts:
\`\`\`diff
...
Tool to compute a diff between two files.
...
system({
    ...
    description: "Tool to compute a diff between two files.",
})
...
-  tool `md_find_files`: Get the file structure of the documentation markdown/MDX files. Return filename, title, description for each match. Use pattern to specify a regular expression to search for in the file content.
...
export default function (ctx: ChatGenerationContext) {
    ...
    defTool(
        "md_find_files",
        "Get the file structure of the documentation markdown/MDX files. Return filename, title, description for each match. Use pattern to specify a regular expression to search for in the file content.",
        {
            type: "object",
            properties: {
                path: {
