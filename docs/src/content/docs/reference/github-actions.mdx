...
description: Learn how to use GitHub Actions to automate your workflows and improve your development process.
sidebar:
    order: 10
---
...
Create a new repository for your action, or use an existing one.
...
Open a terminal in the root of your repository where you have a GenAIScript script you want to package as an action.
...
```bash
npx -y genaiscript configure action <scriptid> --out .
```

where `<scriptid>` is the ID of the script you want to package as an action (filename without `.genai...`).

</li>

<li>

Push the changes, create a release and your action is ready!
...
npx -y genaiscript configure action <scriptid> ... --image <image>
```
...
npx -y genaiscript configure action <scriptid> ... --ffmpeg --playwright
```

You can also add any other packages you need by specifying them in the `cli` command.

```bash "--apks <package1> <package2>"
npx -y genaiscript configure action <scriptid> ... --apks <package1> <package2>
```
