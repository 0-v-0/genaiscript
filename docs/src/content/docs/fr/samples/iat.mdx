...
Le script utilise [workspace.grep](/genaiscript/reference/scripts/files#grep) pour trouver toutes les occurrences du motif regex dans les documents spécifiés.

## Génération du texte alternatif

Pour chaque URL d'image trouvée, nous générons un texte alternatif en utilisant un [prompt inline](/genaiscript/reference/scripts/inline-prompts)
