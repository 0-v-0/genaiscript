...
L'étape de recherche est réalisée avec [workspace.grep](/genaiscript/reference/scripts/files) qui permet de rechercher efficacement un motif dans des fichiers (c'est le même moteur de recherche qui alimente la recherche de Visual Studio Code).
...
La seconde étape consiste à appliquer l'expression régulière au contenu du fichier et à pré-calculer la transformation LLM de chaque correspondance en utilisant un [prompt en ligne](/genaiscript/reference/scripts/inline-prompts).
