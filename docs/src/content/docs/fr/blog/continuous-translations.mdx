...
The LLM is prompted to translate the text between `<ORIGINAL>`et `<TRANSLATED>`, and to return the translated text with the same markers, but with unique identifiers for each translatable chunk.
...
* Le `"mdast-util-mdxjs-esm"`et `"mdast-util-mdx-jsx"` modules add support for MDX features.
...
* Utilise `classify` from GenAIScript Runtime to ask the AI if the translation is good enough.
