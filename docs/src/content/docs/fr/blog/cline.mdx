...
if (files.length === 0) {
  env.output.log(`No TypeScript files found matching pattern: ${env.vars.glob || "**/*.{ts,tsx}"}`);
  cancel("No files to process");
}

env.output.log(`Found ${files.length} TypeScript files to analyze for missing JSDoc comments.`);
...
for (const file of files) {
  ...
  for (const astPattern of astPatterns) {
    try {
      ...
      const { matches } = await sg.search("ts", file.filename, astPattern.pattern);
...
env.output.log(`Found ${allTargets.length} TypeScript declarations missing JSDoc comments.`);
...
- Return ONLY the JSDoc comment block, no other text or code`;
    ...
    const jsdoc = parsers.unfence(jsdocComment.text, "typescript") || jsdocComment.text;
...
if (env.vars.applyEdits) {
  ...
  env.output.log("To apply these changes, run the script with `applyEdits: true`.");
}
```
