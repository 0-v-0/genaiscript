...
Your script should be testable locally using the `genaiscript run` command
and parameters.

```bash
npx -y genaiscript run poem --vars topic=food
```
...
```bash
INPUT_TOPIC=nature npx -y genaiscript run poem
```
