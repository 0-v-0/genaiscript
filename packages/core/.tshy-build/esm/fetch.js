...
import { FETCH_RETRY_DEFAULT, FETCH_RETRY_DEFAULT_DEFAULT, FETCH_RETRY_GROWTH_FACTOR, FETCH_RETRY_MAX_DELAY_DEFAULT, FETCH_RETRY_ON_DEFAULT, } from "./constants.js";
...
export async function createFetch(options) {
    const { retries = FETCH_RETRY_DEFAULT, retryOn = FETCH_RETRY_ON_DEFAULT, trace, retryDelay = FETCH_RETRY_DEFAULT_DEFAULT, maxDelay = FETCH_RETRY_MAX_DELAY_DEFAULT, cancellationToken, } = options || {};
    ...
    const crossFetchWithProxy = agent
        ? (url, options) => crossFetch(url, { ...(options || {}), dispatcher: agent })
        : crossFetch;
    ...
    const fetchRetry = wrapFetch(crossFetchWithProxy, {
        ...
        retryDelay: (attempt, error, response) => {
            ...
            const delay = Math.min(maxDelay, Math.pow(FETCH_RETRY_GROWTH_FACTOR, attempt) * retryDelay) *
                (1 + Math.random() / 20); // 5% jitter for delay randomization
            const msg = prettyStrings(`retry #${attempt + 1} in ${renderWithPrecision(Math.floor(delay) / 1000, 1)}s`, message, status);
            logVerbose(msg);
