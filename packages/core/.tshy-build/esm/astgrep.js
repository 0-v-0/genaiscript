 ...
 */
export function astGrepCreateChangeSet() {
 ...
 */
export async function astGrepFindFiles(lang, glob, matcher, options) {
 ...
 */
export async function astGrepWriteRootEdits(nodes, options) {
 ...
 */
export async function astGrepParse(file, options) {
    const { cancellationToken } = options || {};
    if (file.encoding) {
...
async function resolveLang(lang, filename) {
    ...
    if (typeof lang === "string") {
        ...
        if (builtin)
            ...
            return await loadDynamicLanguage(lang);
    }
    if (!filename) {
    ...
    if (filename) {
        ...
        if (dynamic)
            return await loadDynamicLanguage(dynamic);
