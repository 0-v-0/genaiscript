...
export const vectraWorkspaceFileIndex = async (indexName, cfg, embedder, options) => {
    const { version = 1, deleteIfExists, trace, cancellationToken, chunkSize = 512, chunkOverlap = 128, vectorSize = 1536, } = options || {};
    ...
    return Object.freeze({
        ...
        search: async (query, options) => {
            ...
            const docs = (await index.queryDocuments(query, { maxDocuments: topK })).filter((r) => r.score >= minScore);
            const res = [];
