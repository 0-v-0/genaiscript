 ...
 */
export function generateCliArguments(template, options, command) {
    ...
    if (!isNaN(seed))
        cli.push("--seed", seed + "");
    if (reasoningEffort)
        cli.push("--reasoning-effort", reasoningEffort);
    if (fallbackTools)
        cli.push("--fallback-tools");
    return cli.join(" ");
}
/**
 ...
 */
export function traceCliArgs(trace, template, options) {
    if (isCI)
        return;
    trace.details("🤖 automation", `Use the command line interface [run](https://microsoft.github.io/genaiscript/reference/cli/run/) to automate this task:
