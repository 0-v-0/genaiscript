...
export function createChatGenerationContext(options, trace, projectOptions) {
    ...
    const defTool = (name, description, parameters, fn, defOptions) => {
        ...
        if (typeof name === "string") {
            ...
            const tool = name;
            appendChild(node, createToolNode(tool.spec.name, tool.spec.description, tool.spec.parameters, tool.impl, defOptions, ctx));
        }
    ...
    const runPrompt = async (generator, runOptions) => {
        ...
        try {
            ...
            let prediction;
            // expand template
            const { errors, schemas: scs, tools: fns, messages: msgs, chatParticipants: cps, fileMerges: fms, outputProcessors: ops, fileOutputs: fos, images: imgs, prediction: pred, disposables: dps, } = await renderPromptNode(genOptions.model, node, {
            ...
            if (systemScripts.length)
                try {
                    ...
                    for (const systemId of systemScripts) {
                        ...
                        const sysr = await callExpander(prj, system, mergeEnvVarsWithSystem(env, systemId), runTrace, genOptions, false);
